...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click()
    line = rg.Line(rg.Point(rectangle.get_upper_right_corner().x,rectangle.get_upper_right_corner().y),
                   rg.Point(rectangle.get_lower_left_corner().x,rectangle.get_lower_left_corner().y))
...
def problem2b(rect, n, delta, win):
    ...
    # -------------------------------------------------------------------------
    rect.attach_to(win)
    p1 = rg.Point(rect.get_upper_right_corner().x,rect.get_upper_right_corner().y)
    p2 = rg.Point(rect.get_lower_left_corner().x,rect.get_lower_left_corner().y)
    for k in range(n):
        rect2 = rg.Rectangle(rg.Point(p1.x + delta*(k), p1.y - delta *(k)),
                             rg.Point(p2.x - delta*(k), p2.y + delta*(k)))
        rect2.attach_to(win)
    win.render()
